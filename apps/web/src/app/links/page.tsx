...
function LinksPage() {
  ...
  // Mount + load draft from localStorage (client only)
  useEffect(() => {
    setMounted(true);
    try {
  ...
  // Only populate from DB if no draft exists
  useEffect(() => {
    ...
    <section className="flex flex-col sm:flex-row w-full sm:justify-between gap-6 p-4 lg:p-6 bg-background">
      ...
      <div className="hidden flex-1 lg:flex items-center justify-center bg-card rounded-xl border p-[24px]">
        <div className="relative w-[307px] h-[631px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center pt-24 px-8 space-y-14">
            <div className="w-full flex flex-col items-center">
              <div className="w-24 h-24 rounded-full bg-muted animate-pulse mb-4" />
              <div className="h-2 rounded bg-muted w-3/4 mb-2 animate-pulse" />
              <div className="h-2 rounded bg-muted w-1/2 animate-pulse" />
            </div>
            <div className="w-full overflow-y-auto max-h-[280px] space-y-3 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {isLoading
                ...
                : hasLinks
                  ? links.map((link) => {
                      ...
                      return (
                        <div
                          key={link.id}
                          className="h-12 rounded-lg flex items-center justify-between px-4 transition-colors flex-shrink-0"
      ...
      {/* Customization panel */}
      <div className="flex-1 bg-card rounded-xl border p-6 lg:p-8 flex flex-col">
        <div className="h-[90vh] overflow-auto overflow-y-scroll space-y-[40px]">
          <div>
          ...
          <div className="flex flex-col gap-6 flex-1">
            <button
              onClick={addLink}
              className="flex items-center gap-2 border border-[#633CFF] text-[#633CFF] rounded-[8px] w-full justify-center py-[11px] font-semibold hover:bg-[#EFEBFF] transition-colors cursor-pointer"
            >
              <Plus size={18} />
            ...
            {isLoading && (
              <div className="flex flex-col gap-5">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="rounded-xl border p-5 h-[160px] bg-muted animate-pulse"
                  />
                ))}
              </div>
            )}
            ...
            {/* Empty state */}
            {!isLoading && !hasLinks && (
              <div className="flex flex-col items-center justify-center py-15 px-5 bg-card rounded-xl border space-y-10">
                ...
                <div className="space-y-6">
                  <h2 className="text-2xl lg:text-[32px] font-bold text-center">
                    Let's get you started
                  </h2>
                  <p className="text-[16px] font-normal text-center text-[#737373]">
            ...
            {!isLoading && hasLinks && (
              <div className="flex flex-col gap-5">
                {links.map((link, index) => {
                  const platform = getPlatform(link.platform);
                  const isSaved = !isNaN(Number(link.id));
                  return (
                    <div
                      key={link.id}
                      className="rounded-xl border p-5 space-y-4 transition-colors"
                      style={{ backgroundColor: `${platform.bg}18` }}
                    >
                      {/* Card header */}
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full bg-background border rounded-lg pl-9 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-[#633CFF] transition-shadow duration-200"
                            style={
                              {
                                "--tw-ring-color": platform.bg,
                                boxShadow: undefined,
                              } as React.CSSProperties
                            }
                            onFocus={(e) => {
                              e.currentTarget.style.boxShadow = `0 0 0 4px ${platform.bg}26`;
                            }}
                            onBlur={(e) => {
        ...
        {/* Save button — only render after mount to avoid hydration mismatch */}
        {mounted && hasLinks && (
          <div className="flex justify-end mt-4 pt-4 border-t">
            <button
              onClick={handleSave}
              disabled={addLinkMutation.isPending}
              className="bg-[#633CFF] hover:bg-[#4e2fe0] text-white font-semibold px-6 py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {addLinkMutation.isPending ? "Saving..." : "Save"}
            </button>
          </div>
        )}
      </div>
